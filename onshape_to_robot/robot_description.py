...
def gazebo_fixed_joints(name: str) -> str:
    ...
    etree.SubElement(gazebo, "disableFixedJointLumping").text = "true"
...
class RobotDescription(ABC):
    def __init__(self, robotName: str) -> None:
        ...
        self.xml: str = ""
        self.jointMaxEffort: float = 1.0
        self.jointMaxVelocity: float = 10.0
    ...
    def resetLink(self):
        ...
        self._color = np.array([0.0, 0.0, 0.0, 0.0])
        self._color_mass = 0
        self._link_childs = 0
        self._visuals = []
        self._dynamics = []
...
class RobotURDF(RobotDescription):
    ...
    def append_inertial(
        ...
        if com is not None:
            ...
            etree.SubElement(inertial, "origin", xyz=xyz)
            ...
            for entry in entries:
                if shapes is None:
                    ...
                    for shape in shapes:
                        ...
                        if shape["type"] == "cube":
                            self.append(
                                '<box size="%g %g %g" />' % tuple(shape["parameters"])
                            )
